...
Finally your program should print the total amount of space required to store all of the tracking 
codes (in bytes). If the user enters an incorrect type for any tracking number, the program should print 
...
int main(){
    ...
    int length, sum=0, flag=0;
    char type;
    
    for(int i=0; i<n; i++){
        scanf("%d %c", &length, &type);
        if (type == 'c') sum += (length * 1);
        else if (type == 'i') sum += (length * 4);
        else if (type == 'd') sum += (length * 8);
        else {
            flag = -1;
            i = n;
        }
    ...
    if (flag == -1){
        printf("%s", "Invalid tracking code type");
    }else{
        printf("%d bytes", sum);
    }
